...
class Cleansing:
    def __init__(self):
        ...
        self.company_df.to_csv('./区内乗換NEXT全データ/バス会社.csv', index=False)
    ...
    def company(self):
        ...
        bas_data = pd.read_csv('./csv/乗換案内NEXT区内路線.csv')
        df = pd.DataFrame(bas_data)
        df = df.rename(columns={'会社名称_x': 'バス会社名称', '路線名称_x':'路線名称', 'どこからどこまで_x':'どこからどこまで', '行き方面_x':'行き方面', '帰り方面_x':'帰り方面'})
    ...
    def rosen(self):
        ...
        rosen_id_df = pd.merge(self.company_id_df, df, how='left', on=['路線名称','どこからどこまで','行き方面','帰り方面'])
        # print(self.rosen_id_df.head(30))
        # print(self.rosen_id_df[self.rosen_id_df['路線ID']==2]['バス停名称'].iloc[0])
        # print(self.rosen_id_df[self.rosen_id_df['路線ID']==2])
        # i=1
        # # l=1
        # serial_num = []
        # route_station = []
        # while ((self.rosen_id_df['路線ID']==i).sum()) != 0:
        #     rosen = self.rosen_id_df[self.rosen_id_df['路線ID']==i]
        ...
        #         rosen['路線名称'].replace(rosen_meisyou+'['+rosen['バス停名称'].iloc[0]+']')
        #     elif rosen['バス停名称'].iloc[0] == rosen['バス停名称'].iloc[-1]:
        #         rosen['路線名称'].replace(rosen_meisyou+'['+to_where[0]+']')
        #     else:
        #         rosen['路線名称'].replace(rosen_meisyou+'['+rosen['バス停名称'].iloc[-1]+']')
        #     route_station += rosen.to_dict()
        #     serial_num += list(range(1, (self.rosen_id_df['路線ID']==i).sum()+1, 1))
        #     # l+=rosen.sum()
        #     i+=1
        # df['停車順番'] = serial_num
        ...
        df = pd.DataFrame(bas_data)
        self.teisya_junban = pd.merge(rosen_id_df, df, how='left', on=['路線ID','バス会社ID','バス停ID','バス停名称','路線名称'])
        rosen = self.teisya_junban.loc[:, ['路線ID','停車順番','バス会社ID','路線名称']]
        rosen = rosen.drop_duplicates(subset=['路線ID','バス会社ID','路線名称'])
        rosen.to_csv('./区内乗換NEXT全データ/路線名.csv', index=False)

    def bastei_id(self):
        pd.set_option('display.max_rows', 50)
        bas_data = pd.read_csv('./csv/乗換案内NEXT区内バス停.csv')
        df = pd.DataFrame(bas_data)
        df = df.rename(columns={'区内バス停ID': 'バス停ID'})
        df = df.loc[:, ['バス停ID', '緯度', '経度', 'バス停名称']]
        self.bastei_id_df = pd.merge(self.teisya_junban, df, how='left', on=['緯度','経度', 'バス停名称'])
        self.bastei_id_df = self.bastei_id_df.loc[:, ['路線ID','バス会社ID','バス停ID','バス停名称','路線名称']]
        self.bastei_id_df.to_csv('./区内乗換NEXT全データ/路線データ.csv', index=False)
